...
def start(SNRdb):
    ...
    return  (wrongbits/sequencetobesent.size)

input_values = [i/2 for i in range(0, 81)]

# Calculate the corresponding output values using the 'start' function
output_values = [start(x) for x in input_values]
...
plt.plot(input_values, (plot2), label='BER')
plt.xlabel('SNRdb ')
plt.ylabel(' BER')
plt.title('Plot of BER for  SNRdb Values from 0 to 40')
plt.legend()
plt.show()
...
for i in range(0,len(output_values)):
    if(output_values[i]>0):
        output_values[i]=math.log(output_values[i],10)
    else:
        idx=i
        break

plt.plot(input_values[:idx], output_values[:idx], label='log BER (Note: log BER not plotted for BER=0)')
plt.xlabel('SNRdb')
plt.ylabel('log BER')
plt.title('Plot of log BER for SNRdb Values from 0 to 40')
plt.legend()
plt.show()